...
module.exports = async (context) => {
    ...
    if (!text) {
        return m.reply("❌ *Please provide the note content!*\n\nExample usage:\n`.letter4 _Name_ | _Class or Reg.no or Id no...etc_| _Subject or Main topic_ | _Date (YYYY-MM-DD)_ | _Your note here or paragraph_`");
    }
    ...
    try {
        ...
        if (input.length < 5) {
            return m.reply("❌ *Invalid format!*\n\nExample:\n`.letter4 PETER KANAMBO | XII - Bio A | Sexual Organs | 2025-01-25 | The human reproductive organs consist of...`");
        }

        const [name, classroom, subject, date, content] = input;
        const font = "ShadowsIntoLight-Regular.ttf"; // Default font
        ...
        await client.sendMessage(
            ...
            {
                image: { url: apiUrl },
                caption: `📖 *Here is your ShadowsIntoLight-Regular.ttf handwritten note, ${name}:*\n\n📌 *Subject:* ${subject}\n📅 *Date:* ${date}\n📝 _Handwritten with love_ 💙`,
            },
            { quoted: m }
        );
    } catch (error) {
